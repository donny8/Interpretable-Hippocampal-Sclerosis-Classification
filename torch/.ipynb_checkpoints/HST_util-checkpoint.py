...
def acc_roc(multi,binary,yIdxPrediction,yLabelPrediction,yPredictionB,yLabelPredictionB,Rep,Sum,iters,graph_path, NO4, NO7, NO0, YES1, YES2):    
    ...
    kakao_multi = correct/len(Y_true)
    ...
    kakao_binary = (tn+tp)/(tn+tp+fn+fp)
    kakao_auroc = roc_auc_score(Y_trueB, yPredictionB)
    ...
    result1 = 'Multi : Acc : %0.3f' %(kakao_multi)
    result2 = 'Binary : Acc : %0.3f  AUROC : %0.3f' %(kakao_binary, kakao_auroc)
    kakao(TALK,result1,result2)

    
def kakao(msg, result1, result2):
    target_url = 'https://maker.ifttt.com/trigger/Kakao/with/key/cXQ1G4jB6DyIBqyjdk4EKM'
    r = requests.post(target_url, data={"value1" : msg, "value2" : result1, "value3" : result2})
