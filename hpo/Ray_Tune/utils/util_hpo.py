...
__all__ = ['load_tune_analysis', 'get_best_trial_curve', 'get_trial_order', 'get_trial_curves', 'make_grid', 'get_preeval_results', 'dummy', 'convert_sec']
...
def convert_sec(seconds):
    """
    Function for converting seconds into days, hours, minutes, and seconds
    Input Args:
      seconds (int) : time in sec
    Returns:
      t (str) : converted time
    """
    min = 60
    hr = min * 60
    day = hr * 24
    ...
    days = seconds / day
    hrs = (days - np.floor(days)) * (day / hr)
    mins = (hrs - np.floor(hrs)) * min
    floor = lambda x: int(np.floor(x))

    t = "{} days {} hrs {} mins".format(floor(days), floor(hrs), floor(mins))
    return t
